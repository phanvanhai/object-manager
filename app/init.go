...
import (
	...
	"github.com/object-manager/internal/config"
	...
	for time.Now().Before(until) {
		...
		if Configuration == nil {
			...
			if err != nil {
				...
				LoggingClient = logger.NewClient("object-manager", Configuration.Logging.EnableRemote, logTarget, Configuration.Writable.LogLevel)
...
func initializeConfiguration(useRegistry bool, useProfile string) (*ConfigurationStruct, error) {
	...
	configTree, err := config.LoadFromFile(useProfile, configuration)
	if err != nil {
		return nil, err
	}
	...
	if useRegistry {
		...
		hasConfig, err := registryClient.HasConfiguration()
		if err != nil {
			return nil, err
		}
		...
		if hasConfig {
			...
			err := registryClient.PutConfigurationToml(configTree, true)
			if err != nil {
				return nil, err
			}
...
func initializeClients(useRegistry bool) {
	...
	params.ServiceKey = "Data"
	params.Path = clients.ApiReadingRoute
	params.Url = Configuration.Clients["Data"].Url() + clients.ApiReadingRoute
	clientCoreReading = coredata.NewReadingClient(params, startup.Endpoint{RegistryClient: &registryClient})

	params.Path = clients.ApiValueDescriptorRoute
	params.Url = Configuration.Clients["Data"].Url() + clients.ApiValueDescriptorRoute
	clientCoreValueDescriptor = coredata.NewValueDescriptorClient(params, startup.Endpoint{RegistryClient: &registryClient})

	params.ServiceKey = "Metadata"
	params.Path = clients.ApiDeviceRoute
	params.Url = Configuration.Clients["Metadata"].Url() + clients.ApiDeviceRoute
	clientMetaDevice = metadata.NewDeviceClient(params, startup.Endpoint{RegistryClient: &registryClient})

	params.Path = clients.ApiCommandRoute
	params.Url = Configuration.Clients[services[clients.CoreMetaDataServiceKey]].Url() + clients.ApiCommandRoute
	clientMetaCommand = metadata.NewCommandClient(params, startup.Endpoint{RegistryClient: &registryClient})

	params.Path = clients.ApiDeviceProfileRoute
	params.Url = Configuration.Clients["Metadata"].Url() + clients.ApiDeviceProfileRoute
	clientMetaProfile = metadata.NewDeviceProfileClient(params, startup.Endpoint{RegistryClient: &registryClient})

	params.Path = clients.ApiDeviceServiceRoute
	params.Url = Configuration.Clients["Metadata"].Url() + clients.ApiDeviceServiceRoute
	clientMetaDS = metadata.NewDeviceServiceClient(params, startup.Endpoint{RegistryClient: &registryClient})

	params.Path = clients.ApiAddressableRoute
	params.Url = Configuration.Clients["Metadata"].Url() + clients.ApiAddressableRoute
	clientMetaAddressable = metadata.NewAddressableClient(params, startup.Endpoint{RegistryClient: &registryClient})

	params.ServiceKey = "Command"
	params.Path = clients.ApiDeviceRoute
	params.Url = Configuration.Clients["Command"].Url() + clients.ApiDeviceRoute
	clientCommand = command.NewCommandClient(params, startup.Endpoint{RegistryClient: &registryClient})
}
