...
func LoadFromFile(profile string, configuration interface{}) (tree *toml.Tree, err error) {
	path := determinePath()
	...
	if err != nil {
		return nil, fmt.Errorf("unable to unmarshal configuration tree (%s): %v", path, err.Error())
	}
...
func VerifyTomlFiles(configuration interface{}) error {
	...
	for _, f := range files {
		...
		_, err := LoadFromFile(profile, configuration)
		if err != nil {
